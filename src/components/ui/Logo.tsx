...
const Logo: React.FC<LogoProps> = ({
  ...
  const getSizeDimensions = () => {
    const configs = {
      sm: {
        full: { width: 120, height: 32 },
        compact: { width: 96, height: 32 },
        icon: { width: 32, height: 32 }
      },
      md: {
        full: { width: 160, height: 48 },
        compact: { width: 120, height: 48 },
        icon: { width: 48, height: 48 }
      },
      lg: {
        full: { width: 200, height: 64 },
        compact: { width: 144, height: 58 },
    ...
    return configs[size][variant];
  };

  const dimensions = getSizeDimensions();

  // IMP Digital Services Logo SVG
  const LogoSVG = () => (
    <svg
      ...
      aria-label="IMP Digital Services Logo"
    >
  ...
  const LogoContent = () => (
    <div className={`flex items-center ${className}`}>
      ...
      {/* Optional text for compact variant when showText is true */}
      {variant === 'compact' && showText && (
        <div className="ml-3 hidden sm:block">
          <span className="text-lg font-bold text-charcoal tracking-tight">
            IMP
          </span>
          <span className="text-xs text-charcoal/70 block leading-none">
            Digital
          </span>
        </div>
      )}
  ...
  if (href) {
    return (
      <a 
        href={href}
        className="inline-block focus:outline-none focus:ring-2 focus:ring-matcha focus:ring-offset-2 rounded-lg transition-transform duration-200 hover:scale-105"
        aria-label="IMP Digital Services - Home"
      >
        <LogoContent />
      </a>
    );
